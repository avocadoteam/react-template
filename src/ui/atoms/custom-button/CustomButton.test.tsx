...
describe("Test CustomizedButton component", () => {
  ...
  const primaryBtn = theme.light.btn.primary;
  beforeEach(() => {
    render(
  ...
  test(`background equal to ${primaryBtn.background}`, () => {
    expect(screen.getByTestId(testId).style).toHaveProperty(
      "background",
      primaryBtn.background
    );
  });
  test(`color equal to ${primaryBtn.color}`, () => {
    expect(screen.getByTestId(`${testId}-text`).style).toHaveProperty(
      "color",
      primaryBtn.color
    );
  });
